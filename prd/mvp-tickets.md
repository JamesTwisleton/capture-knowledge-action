...
- Decision provider interface, with two implementations:
  - **Jev adapter:** a thin wrapper written in this codebase that calls the TypeSafe API directly and slots in like the other providers. As far as found, TypeSafe ships JavaScript and Python SDKs and a Python LangChain integration, but no Java SDK, so the adapter calls the HTTP API itself. Uses Jev's choice and score primitives.
  - **LLM fallback:** if no Jev key is configured, the LLM provider makes the same decision through the same interface (less calibrated, same shape).
