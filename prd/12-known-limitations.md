...
| The fallback is a port, and ports drift | The LLM fallback is a Java port of TypeSafe's `system-one-adapter-python`. The contract is theirs, which is the point, but nothing keeps the port in step automatically: an upstream change to the response shape or retry behaviour has to be noticed and applied by hand. Pinned to the upstream commit it was ported from, and re-checked when Jev's API moves. |
